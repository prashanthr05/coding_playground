classdef SE3
    methods (Static)
        ...
        function [Tout, J_minv_m] = inverse(T)
            ...
            J_minv_m = -LieGroups.SE3.AdjointMatrix(T);
        end
        ...
        function [vOut, J_vout_m, J_vout_v] = act(T, vIn)
            ...
            J_vout_m = [R -R*LieGroups.SO3.hat(vIn)];
            J_vout_v = R;
        end
        
        function [T, J_mout_m, J_mout_t] = exphat(v)
            vlin = v(1:3);
            omega = v(4:6);
            ...
            Tinv = LieGroups.SE3.inverse(T);            
            J_mout_m = LieGroups.SE3.AdjointMatrix(Tinv);
            J_mout_t = LieGroups.SE3.rightJacobian(v);
        end
        ...
        function Q = leftJacobianLinearSubmatrix(v)
            ...
            omega_cross_vlin_cross = omega_cross*vlin_cross;
            vlin_cross_omega_cross = vlin_cross*omega_cross;
            omega_cross_vlin_cross_omega_cross = omega_cross*vlin_cross_omega_cross;
            
            a = (theta - sin(theta))/theta_cubed;
            Q = Q + a*(omega_cross_vlin_cross + ...
                vlin_cross_omega_cross + ...
                omega_cross_vlin_cross_omega_cross);
            
            b = (theta_squared + (2*cos(theta)) - 2)/(2*theta_fourth);
            Q = Q + b*(omega_cross*omega_cross_vlin_cross + ...
                vlin_cross_omega_cross*omega_cross - ...
                - 3*omega_cross_vlin_cross_omega_cross);
            
            c = ((2*theta) - (3*sin(theta)) + (theta*cos(theta)))/(2*theta_fifth);
            Q = Q + c*(omega_cross_vlin_cross_omega_cross*omega_cross + ...
                omega_cross*omega_cross_vlin_cross_omega_cross);
        end
